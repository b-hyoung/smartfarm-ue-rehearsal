(function () {
  ...
  // 시작 케이스: 05_F3 에 시각별 원본이 있으면 그것, 없으면 재생되는 첫 케이스.
  function hasFrames(k) { var f = SCENE[k].tiers.t0.frames; return !!(f && Object.keys(f).length); }
  var cur = (SCENE['05_F3'] && hasFrames('05_F3')) ? '05_F3'
          : (KEYS.filter(hasFrames)[0] || (SCENE['05_F3'] ? '05_F3' : KEYS[0]));
