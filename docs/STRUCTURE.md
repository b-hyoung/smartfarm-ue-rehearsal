...
|---|---|
| 실측 (R1) | 임시값이 틀렸는지 판정 |
| PINN | 틀린 입력값(벽 열손실, 풍량 등)을 실측에서 역산 |
| PINO | 보정된 CFD 를 학습해 실시간 응답 (검사 기능 없음) |

## 임시값 → 교체 지점

| 임시값 | 현재 | 파일 | 교체 트리거 |
|---|---|---|---|
| 온도·기류 예측 | 수식 모델 | `src/predict_mock.py` | 케이스 라이브러리 → PINO |
| 조명 발열 | 480 W (240×2단) | `src/vane_mock.py` LIGHT_W | 스펙시트 소비전력 |
| 배광 | 반각 35° 근사 | `web/3d.html`, UE RectLight | IES 파일 (LM-63) |
| 냉방 COP | 3.5 (1등급 추정) | `src/power_model.py` COP | 전력 실측 |
